...
if [ -f "$specMeshDir/bootstrap.sh" ]; then
...
rootPackage=$(<.specmesh/service_template/root.package)

# sedCode(sedCmd)
function sedCode() {
  find . -type f -not \( -path "./init.sh" -o -path "./init_headless.sh" -o -path "*/.git/*" -o -path "*/build/*" -o -path "*/.gradle/*" -o -path "*/.specmesh/*" \) -print0 | xargs -0 sed -i "$1"
}
...
cp -R "$specMeshDir/service_template/services" "./"
...
cp -R "$specMeshDir/service_template/example-service" "$serviceName"
